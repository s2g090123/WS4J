...
/**
 * Test class for [WuPalmer].
 *
 * @author Hideki Shima
 */
class WuPalmerTest : RelatednessCalculatorTest() {
    @Test
    override fun testHappyPathOnSynsets() {
        assertEquals(0.9565, rc.calcRelatednessOfSynsets(cycloneConcepts[1], hurricaneConcepts[0]).score, 0.0001)
        assertEquals(0.6957, rc.calcRelatednessOfSynsets(cycloneConcepts[0], hurricaneConcepts[0]).score, 0.0001)
        assertEquals(0.8571, rc.calcRelatednessOfSynsets(migrateConcepts[0], emigrateConcepts[0]).score, 0.0001)
        assertEquals(0.5714, rc.calcRelatednessOfSynsets(migrateConcepts[1], emigrateConcepts[0]).score, 0.0001)
    }
    ...
    override fun testHappyPathOnWords() {
        assertEquals(0.9565, rc.calcRelatednessOfWords(CYCLONE, HURRICANE), 0.0001)
        assertEquals(0.8571, rc.calcRelatednessOfWords(MIGRATE, EMIGRATE), 0.0001)
    }
    ...
    override fun testHappyPathOnWordsWithPOS() {
        ...
        assertEquals(
            0.0000, rc.calcRelatednessOfWords(
                CHAT + WordSimilarityCalculator.SEPARATOR + POS.NOUN,
