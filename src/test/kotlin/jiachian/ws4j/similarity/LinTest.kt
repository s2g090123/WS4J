 ...
 * @author Hideki Shima
 */
class LinTest : RelatednessCalculatorTest() {
    @Test
    override fun testHappyPathOnSynsets() {
        assertEquals(0.9813, rc.calcRelatednessOfSynsets(cycloneConcepts[1], hurricaneConcepts[0]).score, 0.0001)
        assertEquals(0.0000, rc.calcRelatednessOfSynsets(cycloneConcepts[0], hurricaneConcepts[0]).score, 0.0001)
        assertEquals(0.9463, rc.calcRelatednessOfSynsets(migrateConcepts[0], emigrateConcepts[0]).score, 0.0001)
    ...
    override fun testHappyPathOnWords() {
        assertEquals(0.9813, rc.calcRelatednessOfWords(CYCLONE, HURRICANE), 0.0001)
        assertEquals(0.9463, rc.calcRelatednessOfWords(MIGRATE, EMIGRATE), 0.0001)
    }
